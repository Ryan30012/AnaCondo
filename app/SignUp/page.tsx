...
const SignUp = () => {
    ...
    async function signup(formData: FormData){
      ...
      const exists = (await sql`select Fname from users where Fname = ${Fname}`).rowCount > 0;
      console.log(exists)
      if(!exists) {
      try {
        const result = await sql`insert into users(Fname, Lname, Dob, Address, Phone, Email, Password, Utype) values (${Fname},${Lname},${DOB},${Address},${Phone},${Email},${Password},${uType})`;
      }
      catch(error) {
        console.log(error);
      }
...
<div className="bg-stone-50 flex flex-col items-center justify-center h-screen">
  <form className="bg-stone 50 w-full max-w-md" action={signup}>
    ...
    <div className="mb-6">
      <button type="submit" className="w-full text-black bg-blue-500 text-white py-2 px-4 rounded hover:bg-blue-700 focus:outline-none focus:shadow-outline-blue">
        Sign Up
      </button>
    </div>
