...
export async function POST(request: Request) {
    ...
    const common_room = data.get("common-room") as string;
    const deficiency = data.get("message") as string;
    ...
    if (validEmail === null || validEmail === undefined) {
        ...
        try {
            await sql`INSERT INTO "submittedrequests" (request_id, request_type, user_email, request_status, common_room, deficiency) VALUES (${pk}, 'DEFICIENCY', ${user_email}, 'Pending', ${common_room}, ${deficiency});`;
            console.log("Successfully added row in submittedrequests table");
        } catch (error) {
            throw new Error(`Yikes! We ran into an error: ${error}`);
        }
        console.log("DEFICIENCY COMPLETED");
    }  
