...
export async function POST(request: Request) {
    ...
    const unit_nb = data.get("unitNb") as string;
    ...
    if (validEmail === null || validEmail === undefined) {
        ...
        try {
            await sql`INSERT INTO "submittedrequests" (request_id, request_type, user_email, request_status, unit_nb) VALUES (${pk}, 'INTERCOM', ${user_email}, 'Pending', ${unit_nb});`;
            console.log("Successfully added row in submittedrequests table");
        } catch (error) {
            throw new Error(`Yikes! We ran into an error: ${error}`);
        }
        console.log("INTERCOM COMPLETED");
    }  
