...
export async function POST(request: Request) {
    ...
    const violation = data.get("violation") as string;
    ...
    if (validEmail === null || validEmail === undefined) {
        ...
        try {
            await sql`INSERT INTO "submittedrequests" (request_id, request_type, user_email, request_status, violation) VALUES (${pk}, 'VIOLATION', ${user_email}, 'Pending', ${violation});`;
            console.log("Successfully added row in submittedrequests table");
        } catch (error) {
            throw new Error(`Yikes! We ran into an error: ${error}`);
        }
        console.log("VIOLATION COMPLETED");
    }  
