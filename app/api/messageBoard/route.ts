...
export async function POST(req: Request) {
  ...
  try {
    console.log(1);
    const create = await sql`CREATE TABLE IF NOT EXISTS BMessage(
    ...
    console.log(2);
    const result =
      await sql`INSERT INTO BMessage (date, email, message) VALUES(${date},${email},${message})`;
    console.log(3);
...
export async function PUT(req: Request): Promise<NextResponse> {
  try {
    const { id, email, date, message } = await req.json();
